    ...
    try {
        ...
        if (result.type === 'content') {
            ...
            try {
                ...
                try {
                    const existing = await pb.collection('tiktok_audience_demographics').getFirstListItem(
                        `type = "${result.subtype}" && country = "${country}" && date_reference >= "${today} 00:00:00"`,
                        { requestKey: null }
                    );
                    await pb.collection('tiktok_audience_demographics').update(existing.id, recordData, { requestKey: null });
                } catch (err) {
                    if (err.status === 404 || err.message === 'Not found') {
            ...
            try {
                ...
                try {
                    const existing = await pb.collection('tiktok_audience_demographics').getFirstListItem(
                        `type = "activity" && country = "${country}" && date_reference >= "${today} 00:00:00"`,
                        { requestKey: null }
                    );
                    await pb.collection('tiktok_audience_demographics').update(existing.id, recordData, { requestKey: null });
                } catch (err) {
    ...
    try {
        ...
        if (result.type === 'metric') {
            ...
            try {
                ...
                // Check duplicate for today
                try {
                    const existing = await pb.collection('facebook_audience_demographics').getFirstListItem(
                        `platform = "facebook" && country_filter = "${country}" && import_date >= "${today} 00:00:00"`,
                        { requestKey: null }
                    );
                    await pb.collection('facebook_audience_demographics').update(existing.id, recordData, { requestKey: null });
                } catch (err) {
                    if (err.status === 404 || err.message === 'Not found') {
